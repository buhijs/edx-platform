...
class CompletionBlockUpdateViewTestCase(SharedModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.client = APIClient()
        self.client.force_authenticate(user=self.test_user)
        self.update_url = '/api/completion/v1/course/{}/blocks/{}/'.format(self.course.id, self.usage_key)

    def test_create_view(self):
        completion_query_url = '/api/completion/v1/course/edX/toy/2012_Fall/?requested_fields=sequential'
        before_response = self.client.get(completion_query_url)
        self.assertEqual(before_response.status_code, 200)

        before_data = before_response.data
        self.assertEqual(before_data['completion']['earned'], 0)
        self.assertEqual(before_data['completion']['possible'], 12)
        self.assertEqual(before_data['completion']['percent'], 0)
    ...
    def test_create_view_oauth2(self):
        ...
        """
        url = '/api/completion/v1/course/{}/blocks/{}/'.format(self.course.id, self.usage_key)
