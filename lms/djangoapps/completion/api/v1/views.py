...
from ...models import AggregateCompletion, BlockCompletion
...
class CompletionViewMixin(object):
    ...
    def get_aggregations(self, queryset=None):
        """
        Return a dict mapping aggregation_names to a list of aggregations of that name in the
        """
        if queryset is None:
            queryset = self.get_progress_queryset()
        result = defaultdict(list)
        for aggregation in queryset:
            result[aggregation.aggregation_name].append(aggregation)
        return result
...
class CompletionListView(CompletionViewMixin, APIView):
    ...
    **Returns**
    ...
    def get(self, request):
        ...
        aggregations_queryset = self.get_progress_queryset().filter(
    ...
    **Returns**
    ...
    def get(self, request, course_id):
        ...
        try:
            ...
            completion = self.get_progress_queryset().filter(course_key=course_key)
